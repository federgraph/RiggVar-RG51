...
type
  ...
  private
    ...
    colorStart: TBGRAPixel;
    colorEnde: TBGRAPixel;
...
begin
  ...
  colorStart := CssPlum;
  colorEnde := CssAntiquewhite;

  P0 := TRggCircle.Create('P0');
  P0.StrokeColor := CssOrangered;

  P := TRggCircle.Create('P');
  P.StrokeColor := CssOrangered;
  P.IsComputed := True;
  ...
  Arc.StrokeColor := CssPlum;
  Add(Arc);

  KK := TRggPolyCurve.Create('KK', 50);
  KK.StrokeThickness := 3;
  KK.StrokeColor := CssAquamarine;
  KK.ShowCaption := True;
  Add(KK);
  ...
  Chart.StrokeColor := CssDodgerblue;
  Chart.InitDefault;
  Chart.Box.X := 250;
  Chart.Box.Y := 250;
  Chart.Box.Width := 400;
...
begin
  ...
  begin
    angle := psiA + i * step;
    GetLocalC(angle);
    KK.Poly[i].X := Round(localC.X);
    KK.Poly[i].Y := Round(localC.Y);
  end;
end;
