...
{$endif}
    ...
    procedure InplaceShapeMouseDown(Sender: TObject; Button: TMouseButton; Shift: TShiftState; X, Y: Integer);
    procedure InplaceShapeMouseMove(Sender: TObject; Shift: TShiftState; X, Y: Integer);
    procedure InplaceShapeMouseUp(Sender: TObject; Button: TMouseButton; Shift: TShiftState; X, Y: Integer);

{$ifdef WantDrawingList}
    procedure DrawingListClick(Sender: TObject);
    procedure DrawingListKeyUp(Sender: TObject; var Key: Word; Shift: TShiftState);
{$endif}
    procedure ElementListClick(Sender: TObject);
    procedure ElementListKeyUp(Sender: TObject; var Key: Word; Shift: TShiftState);
...
procedure TFormDrawing.FormMouseWheel(Sender: TObject; Shift: TShiftState;
  WheelDelta: Integer; MousePos: TPoint; var Handled: Boolean);
begin
  ImageMouseWheel(Sender, Shift, WheelDelta, Handled);
end;
...
begin
  ...
  GlobalShowCaptionBtn.AllowAllUp := True;
  GlobalShowCaptionBtn.Down := TRggElement.GlobalShowCaption;

  InitComponentProps;
end;

procedure TFormDrawing.InitBitmapAndImage;
var
  b: TBitmap;
begin
...
begin
{$ifdef WantDrawingList}
  DrawingList := TListBox.Create(Self);
  DrawingList.Parent := Self;
  DrawingList.OnClick := DrawingListClick;
  DrawingList.OnKeyUp := DrawingListKeyUp;
{$endif}

  ElementList := TListBox.Create(Self);
  ElementList.Parent := Self;
  ElementList.OnKeyUp := ElementListKeyUp;
  ElementList.OnClick := ElementListClick;
  ...
  InplaceShape.Shape := stCircle;
  InplaceShape.Brush.Style := bsClear;
...
procedure TFormDrawing.FormKeyPress(Sender: TObject; var Key: Char);
begin
  case Key of
    'C': SwapColorScheme;
...
begin
  case Key of
    vk_Escape: DoReset;
    vk_F1: ShowInfo;
    vk_F2: SwapColorScheme;
    vk_F3: SwapThickLines;
    vk_F6: SwapDrawingLists;
    vk_F11: SwapLayout;
  end;
end;
...
begin
  c.Width := ClientWidth - c.Left - Margin;
...
procedure TFormDrawing.ImageMouseDown(Sender: TObject; Button: TMouseButton; Shift: TShiftState; X, Y: Integer);
begin
  MouseDown := True;
  MousePos.X := X;
  MousePos.Y := Y;
...
procedure TFormDrawing.ImageMouseMove(Sender: TObject; Shift: TShiftState; X, Y: Integer);
var
 dx, dy: Integer;
begin
...
procedure TFormDrawing.ImageMouseUp(Sender: TObject; Button: TMouseButton; Shift: TShiftState; X, Y: Integer);
begin
  MouseDown := False;
end;
...
procedure TFormDrawing.InplaceShapeMouseDown(Sender: TObject; Button: TMouseButton; Shift: TShiftState; X, Y: Integer);
begin
...
procedure TFormDrawing.InplaceShapeMouseMove(Sender: TObject; Shift: TShiftState; X, Y: Integer);
var
  dx, dy, dt: Integer;
...
{$ifdef WantDynamicFixPoint}
  ...
  begin
    ML.Add(Format('dx = %d', [dx]));
    ML.Add(Format('dy = %d', [dy]));
  end;
...
procedure TFormDrawing.InplaceShapeMouseUp(Sender: TObject; Button: TMouseButton; Shift: TShiftState; X, Y: Integer);
begin
...
//  d := Screen.DisplayFromForm(self);
  wa := Screen.WorkAreaRect; // d.WorkArea;
...
{$ifdef WantDrawingList}
procedure TFormDrawing.DrawingListClick(Sender: TObject);
begin
  if DrawingList.ItemIndex = -1 then
    Exit;
  SelectDrawing(DrawingList.ItemIndex);
end;

procedure TFormDrawing.DrawingListKeyUp(Sender: TObject; var Key: Word; Shift: TShiftState);
begin
...
procedure TFormDrawing.ElementListClick(Sender: TObject);
var
  ii: Integer;
begin
...
procedure TFormDrawing.ElementListKeyUp(Sender: TObject; var Key: Word; Shift: TShiftState);
begin
  if Key = VK_Space then
...
procedure TFormDrawing.DrawToCanvas(g: TBGRABitmap);
var
{$ifdef WantDynamicFixPoint}
  cr: TRggCircle;
{$endif}
...
{$else}
  ...
  ML.Add(Format('Image  = (%d, %d)', [Image.Width, Image.Height]));
...
begin
  ...
  begin
    InplaceShape.Pen.Color := CssAntiquewhite;
  end
  else
  begin
    InplaceShape.Pen.Color := clBlack;
  end;
